...
export function decodeProposalId(proposalId: string): { pluginAddress: string; id: number } {
    if (!isProposalId(proposalId)) {
        throw new Error("Invalid proposalId");
    }

    const matchedRegexResult = proposalId.match(/^(0x[A-Fa-f0-9]{40})_(0x[A-Fa-f0-9]{1,64})$/) || [];
