...
export class FakerRelayServer {
        ...
        this.app.post(
            ...
            [
                body("email")
                    .exists()
                    .matches(/^(0x)[0-9a-f]{64}$/i),
                body("amountPoint").custom(Utils.isAmount),
                body("signer")
                    .exists()
                    .isEthereumAddress(),
                body("signature")
    ...
    private async exchangeTokenToPoint(req: express.Request, res: express.Response) {
        console.log(`POST /exchangeTokenToPoint`);
    ...
    private async exchangePointToToken(req: express.Request, res: express.Response) {
        console.log(`POST /exchangePointToToken`);
        ...
        try {
            ...
            console.log(`TxHash(exchangePointToToken): `, tx.hash);
            return res.status(200).json(this.makeResponseData(200, { txHash: tx.hash }));
        } catch (error) {
            let message = ContractUtils.cacheEVMError(error as any);
            if (message === "") message = "Failed exchange Point To Token";
            console.error(`POST /exchangePointToToken :`, message);
            return res.status(200).json(
                this.makeResponseData(500, undefined, {
                    message
                })
