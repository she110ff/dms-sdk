export class ResponseMessage {
    ...
    private static find1_length = ResponseMessage.find1_message.length;
    private static find2_message = "reverted with reason string";
    private static find2_length = ResponseMessage.find2_message.length;
    public static cacheEVMError(root: any): string {
        ...
        while (error !== undefined) {
            if (error.reason) {
                ...
                let idx = reason.indexOf(ResponseMessage.find1_message);
                let message: string;
                if (idx >= 0) {
                    message = reason.substring(idx + ResponseMessage.find1_length).trim();
                    reasons.push(message);
                }
                idx = reason.indexOf(ResponseMessage.find2_message);
                if (idx >= 0) {
                    message = reason.substring(idx + ResponseMessage.find2_length).trim();
                    reasons.push(message);
                }
            }
            error = error.error;
